...
def group_groupSkel_changed_handler(request, event):
	if request.user.is_staff:
		...
		yield utils.notify(_(u'Group "{0}" now depends on skel {1}.').format(group.name, group.groupSkel))
...
def group_member_deleted_handler(request, event):
	...
	if request.user.is_staff:
		...
		yield utils.format_RPC_JS('update_relationship', 'user',
								user.uidNumber,
								group.standard_group.gidNumber
									if group.is_helper else group.gidNumber,
								0)
