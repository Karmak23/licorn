...
def delete_user_parse_arguments(app) :
	...
	user.add_option("--no-archive",
		...
		help="Don't make a backup of user's home directory in %s (default is %s)." % (styles.stylize(styles.ST_PATH, configuration.home_archive_dir), styles.stylize(styles.ST_DEFAULT, "to make a backup")))
...
def modify_configuration_parse_arguments(app) :
	...
	usage_text = "\n\t%s config[uration] [--set-hostname <new hostname>] [--restrictive] [--set-ip-address <NEW.ETH0.IP.ADDR>]" % styles.stylize(styles.ST_APPNAME, "%prog")
	...
	configuration = OptionGroup(parser, styles.stylize(styles.ST_OPTION, "Modify configuration options "))

	configuration.add_option("--setup-shared-dirs",
	...
	configuration.add_option("-r", "--restrictive", "--set-restrictive",
