...
class User(CoreStoredObject, CoreFSUnitObject):
	...
	@staticmethod
	def make_login(lastname='', firstname='', inputlogin='', force_badname=False):
		...
		if force_badname:
			login_regex     = hlstr.cregex['bad_login']
			login_regex_str = hlstr.regex['bad_login']

		else:
			login_regex     = hlstr.cregex['login']
			login_regex_str = hlstr.regex['login']

		if not login_regex.match(login):
			if inputlogin:
				raise exceptions.BadArgumentError(_(u'Cannot validate the '
								u'provided login "{0}" against /{1}/.').format(
									login, login_regex_str))

			else:
				raise exceptions.BadArgumentError(_(u'Cannot build a valid '
					u'login (got "{0}", which does not verify /{1}/) with the '
					u'firstname/lastname you provided ("{2}" and "{3}").').format(
						login, login_regex_str, firstname, lastname))
		...
		with self.lock:
			if lock:
				if self.__locked:
					...
					LicornEvent('user_pre_lock', user=self.proxy).emit(synchronous=True)

					self.__userPassword = '!' + self.__userPassword

					LicornEvent('user_post_lock', user=self.proxy).emit(synchronous=True)
				...
				if self.__locked:
					...
					LicornEvent('user_pre_unlock', user=self.proxy).emit(synchronous=True)

					self.__userPassword = self.__userPassword[1:]

					LicornEvent('user_post_unlock', user=self.proxy).emit(synchronous=True)
...
class UsersController(DictSingleton, CoreFSController, SelectableController):
	...
	def load(self):
		...
		LicornEvent('users_loading', users=self).emit(synchronous=True)

		assert ltrace(TRACE_USERS, '| load()')
		self.reload(send_event=False)

		LicornEvent('users_loaded', users=self).emit(synchronous=True)
	...
	def reload(self, send_event=True):
		...
		if send_event:
			LicornEvent('users_reloading', users=self).emit(synchronous=True)
		...
		if send_event:
			LicornEvent('users_reloaded', users=self).emit(synchronous=True)
	...
	def __validate_basic_fields(self, login, force_badname, firstname, lastname,
															gecos, shell, skel):
		...
		if not login_regex.match(login):
			if login_autogenerated:
				...
				raise exceptions.BadArgumentError(_(u'Malformed login "{0}", '
					'must match /{1}/.').format(login, login_regex_str))
	...
	def add_User(self, login=None, system=False, password=None, gecos=None,
		...
		force_badname=False, disabled_password=False):
		...
		login, firstname, lastname, gecos, shell, skel = \
			self.__validate_basic_fields(login, force_badname, firstname,
				lastname, gecos, shell, skel)
		...
		with self.lock:
			...
			# Autogenerate password if not given and not disabled_password.
			...
			LicornEvent('user_pre_add', uid=uid, login=login,
								system=system, password=None
									if disabled_password
									else password).emit(synchronous=True)
			...
			create_kwargs = dict(uidNumber=uid,
								login=login,
								gecos=gecos,
			...
			if disabled_password:
				create_kwargs['userPassword'] = '!'
			else:
				create_kwargs['password'] = password

			user = self[uid] = User(**create_kwargs)
			...
			user.check(initial=True, minimal=True, skel_to_apply=skel_to_apply,
																	batch=True)
		...
		if gen_pass:
			if disabled_password:
				logging.notice(_(u'{0}, the user will not '
								u'able to login from console.').format(
									stylize(ST_BAD, _('Password disabled'))))
		...
		logging.notice(_(u'Created {accttype} user {login} (uid={uid}) in '
				u'backend {backend}.').format(accttype=_(u'system')
												if system else _(u'standard'),
					login=stylize(ST_LOGIN, login),
					uid=stylize(ST_UGID, uid),
					backend=backend.pretty_name))
	...
	def del_User(self, user, no_archive=False, force=False, batch=False):
		...
		LicornEvent('user_post_del', uid=uid, login=login, system=system,
						no_archive=no_archive).emit(synchronous=True)
