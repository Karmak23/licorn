...
from licorn.core.tasks                      import ExtinctionTask
	...
	for task in LMC.tasks:
		if isinstance(task, ExtinctionTask):
			try:
				...
				# can be raised if an ExtinctionTask has not the name we expect
				pass
...
def get_calendar_data(request, ret=True):
	...
	for rule in LMC.tasks:
		if isinstance(rule, ExtinctionTask):
			already_added = False
			for r in data_sep:
				if r == rule:
					#same, pass
...
def get_recap(request):
	...
	extinction_tasks = [ t for t in LMC.tasks
									if isinstance(t, ExtinctionTask) ]

	if extinction_tasks:
		return HttpResponse(render_to_string('energy/recap_policies.html', {
				'tasks'    : sorted(
								sorted(
									sorted(extinction_tasks,
										key=attrgetter('minute')
									),
									key=attrgetter('hour')
								),
								key=attrgetter('week_day')
							),
				'get_days' : get_days,
				'generate_machine_html' : generate_machine_html,
			}))

	return HttpResponse('<h3>%s</h3>' % _('No extinction task created yet.'))
