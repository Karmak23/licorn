...
def get_users_parse_arguments(app, configuration):
	...
	return parser.parse_args()
def get_privileges_parse_arguments(app, configuration):
	""" Integrated help and options / arguments for « get user(s) »."""
	...
	return parser.parse_args()
def get_groups_parse_arguments(app, configuration):
	...
	return parser.parse_args()
def get_keywords_parse_arguments(app, configuration):
	...
	return parser.parse_args()
def get_profiles_parse_arguments(app, configuration):
	...
	return parser.parse_args()
def get_machines_parse_arguments(app, configuration):
	""" Integrated help and options / arguments for « get user(s) »."""
	...
	return parser.parse_args()
def get_configuration_parse_arguments(app, configuration):
...
def add_user_parse_arguments(app, configuration):
	...
	user.add_option('-p', "--password",
		...
		help="""Specify user's password (else will be autogenerated, %d """
			"""chars long).""" % configuration.users.min_passwd_size)
	...
	user.add_option('-H', "--home",
		...
		help="""Specify the user's home directory. Only valid for a system """
			...
			"""fixed home dir %s/<login>""" % configuration.users.base_path)
	user.add_option("-S", "--shell",
		...
		help="""Specify user's shell, from the ones given by command """
			...
			configuration.users.default_shell)
	user.add_option('-K', "--skel",
		...
		help="""Specify a particular skeleton to apply to home dir after """
			...
			"""else %s.""" % configuration.users.default_skel)
