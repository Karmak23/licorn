...
function drag_enter(event) {
	...
	num_drag_event++;
    target.addClass('upload-area-hover');
...
function drag_leave(event) {
	...
	num_drag_event--;
	if (num_drag_event == 0) {
		target.removeClass('upload-area-hover');
	}
...
// Issue with dragleave which repeats on hover:
// http://bugs.jquery.com/ticket/11801
			...
			if(xhr.upload){
				xhr.upload.addEventListener('progress', function (e) {
					if (e.lengthComputable) {
						...
						// Upload succeeded, show result and remove it
						// after 5 seconds, else the result area will
						// be crowded with old result.
						setTimeout( function(){
							recap_line.slideUp('slow', function(){
								$(this).remove();
							});
						}, 5000);
					}
