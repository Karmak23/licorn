...
class DonutPlot(PiePlot):
    def __init__ (self,
        ...
        Plot.__init__(self, surface, data, width, height, background,
                        series_colors=series_colors, border=-1)
        self.center = (self.width/2, self.height/2)
        self.total  = sum(self.data)
        #self.radius = min(self.width/3,self.height/3)
        self.radius = radius
...
def donut_plot(name, data, width, height, background=None, gradient=False,
                shadow=False, colors=None, inner_radius=-1, radius=100):
    ...
    plot = DonutPlot(name, data, width, height, background, gradient, shadow, colors, inner_radius, radius)
    plot.render()
    plot.commit()
