...
class KeywordsController(Singleton, LockedController):
	...
	def __init__(self):
		...
		self.work_path     = os.getenv('LICORN_KEYWORDS_PATH',
										LMC.configuration.groups.base_path)
		#
		# TODO: work_path could be HOME if fsapi.minifind is configured to
		# follow symlinks, this would be more optimized than to walk
		# /home/groups (because user has small prob to be in all groups).
		#
		# TODO: implement work_path with more than one path
		#		 (/home/groups:/home/olive:/data/special)
		#
	...
	def WriteConf(self):
		...
		if self.changed :
			logging.progress(_(u'%s: saving data structures to disk.') %
								self.pretty_name)
			...
			logging.info(_(u'%s: data structures saved to disk.') %
								self.pretty_name)
	...
	def ExportXML(self):
		...
		<description>%s</description>
			...
			self.keywords[name]['description'])
...
""" % '\n'.join(map(build_output, self.keywords.keys()))
	def AddKeyword(self, name = None, parent = "", description = ""):
		...
		if name in self.keywords.keys():
			raise exceptions.AlreadyExistsException(
				_(u'A keyword named "%s" already exists!') % name)

		if parent != '':
			if parent not in self.keywords.keys():
				raise exceptions.BadArgumentError(_(u'The parent you '
										u'specified does not exist.'))

		if not hlstr.cregex['keyword'].match(name):
			raise exceptions.BadArgumentError(_(u'Malformed keyword name '
		...
		if not hlstr.cregex['description'].match(description):
			raise exceptions.BadArgumentError(_(u'Malformed keyword '
				u'description "{0}", must match /{1}/i.').format(
		...
		logging.info(_(u'Added keyword %s.') % name)
		self.changed = True
		self.WriteConf()
		...
		try:
			...
			if children != []:
				if del_children:
					...
					raise exceptions.LicornException(_(u'The keyword is a '
						u'parent which has children. If you want to delete '
						u'children too, use option --del-children.'))
			del(self.keywords[name])

			logging.info(_(u'Deleted keyword %s.') % name)
			self.changed = True
			self.WriteConf()
			...
			raise exceptions.BadArgumentError(_(u'The keyword you specified '
												u'does not exist.'))
	...
	def ChangeParent(self, name, parent):
		...
		try:
			...
			raise exceptions.BadArgumentError(_(u'The keyword %s does not '
												u'exist.') % e)
		self.changed = True
		...
		def __add_keywords_to_file(file_path):
			...
			try:
				...
				if e.errno in (61, 95):
					...
					logging.warning2(_(u'Cannot get current keywords.'), once=True)
		...
		def __delete_keywords_from_file(file_path):
			...
			try:
				if keywords_to_set == []:
					...
					logging.info(_(u'Removed xattr {0} from {1}.').format(
								stylize(ST_NAME, self.licorn_xattr),
								stylize(ST_PATH, file_path)))
				else:
					attr = ','.join(self.__remove_bad_keywords(keywords_to_set))
				...
				if e.errno in (1, 95): # Operation not permitted / not supported
					logging.warning(_(u'Unable to modify {0} xattr of {1} '
									u'(was: {2}).').format(
										stylize(ST_NAME, self.licorn_xattr),
										stylize(ST_PATH, file_path), e))
				else:
	...
	def ClearKeywords(self, path, recursive=False):
		...
		if os.path.isfile(path):
			...
			logging.info(_(u'Removed xattr from %s.') % stylize(ST_PATH, path))
