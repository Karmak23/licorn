...
class UsersController(Singleton, Pyro.core.ObjBase):
	...
	def __init__(self, configuration):
		...
		UsersController.backends = UsersController.configuration.backends
	...
	def AddUser(self, login=None, system=False, password=None, gecos=None,
		...
		if desired_uid is None:
			if system:
				uid = pyutils.next_free(self.users.keys(),
					UsersController.configuration.users.system_uid_min,
					UsersController.configuration.users.system_uid_max)
			else:
				uid = pyutils.next_free(self.users.keys(),
					UsersController.configuration.users.uid_min,
					UsersController.configuration.users.uid_max)
		...
		if password is None:
			...
			password = hlstr.generate_password(
				UsersController.configuration.users.min_passwd_size)
	...
	def CheckUsers(self, uids_to_check = [], minimal=True, batch=False,
		...
		UsersController.configuration.check_base_dirs(minimal=minimal,
			...
			if  uid >= self.configuration.users.system_uid_min \
				...
				if os.path.exists(self.users[uid]['homeDirectory']):
					...
					all_went_ok &= fsapi.check_dirs_and_contents_perms_and_acls(
						...
						allgroups=UsersController.groups, allusers=self,
						listener=listener)
	...
	def confirm_uid(self, uid):
		...
		try:
			return UsersController.users[uid]['uidNumber']
		except KeyError:
			try:
				# try to int(), in case this is a call from cli_select(), which
				# gets only strings.
				return UsersController.users[int(uid)]['uidNumber']
			except ValueError:
